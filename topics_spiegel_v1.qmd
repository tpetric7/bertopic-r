...
rep_docs <- get_most_representative_docs(df_docs, topic_nr = 3, n_docs = 5)
unique(rep_docs)
```
...
```{r}
find_topics_df(topic_model, queries = c("migranten", "asylanten"))
```
...
```{r}
visualize_hierarchy(model = topic_model, auto_open = FALSE)
```
...
visualize_hierarchy(model = topic_model, 
                    ...
                    filename = "topic_hierarchy", # default name
                    auto_open = FALSE) # TRUE enables output to browser
```
...
visualize_documents(topic_model, texts = texts_cleaned, reduced_embeddings = reduced_embeddings, auto_open = FALSE)
```
...
If our dataset includes categorical variables (groups, classes, etc.), we can use the `visualize_topics_per_class()` function to display an interactive barchart with the groups or classes associated with a chosen topic. With a double-click of the mouse, the user can display a topic and the frequency of the groups.
